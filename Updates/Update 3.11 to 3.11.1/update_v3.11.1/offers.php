...
try {
  ...
  $total = $db->query("SELECT COUNT(*) as count " . $distance_max_clause . " FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id " . $distance_join . " WHERE posts_offers.end_date >= CURDATE()" . $where_query . $distance_query . $order_query) or _error('SQL_ERROR');
