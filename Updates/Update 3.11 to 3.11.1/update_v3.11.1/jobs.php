...
try {
  ...
  $total = $db->query("SELECT COUNT(*) as count " . $distance_max_clause . " FROM posts INNER JOIN posts_jobs ON posts.post_id = posts_jobs.post_id " . $distance_join . " WHERE posts_jobs.available = '1'" . $where_query . $distance_query . $order_query) or _error('SQL_ERROR');
