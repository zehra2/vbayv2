...
class User
{
  ...
  public function get_currencies()
  {
    global $db, $system;
    $currencies = [];
    $get_currencies = $db->query("SELECT * FROM system_currencies WHERE enabled = '1' ORDER BY currency_id ASC") or _error("SQL_ERROR_THROWEN");
    ...
    if ($get_stories->num_rows > 0) {
      while ($_story = $get_stories->fetch_assoc()) {
        ...
        while ($media_item = $get_media_items->fetch_assoc()) {
          ...
          $story_item['preview'] = $system['system_uploads'] . '/' . $media_item['source'];
          $story_item['link'] = '.';
